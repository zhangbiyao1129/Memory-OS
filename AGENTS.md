...
> 本文件只记录 Memory OS 项目特例。通用交互、代码改动、验证、Review、安全、Git、Memory 规则以全局 `~/ai-rules/AGENTS.md` 为准，此处不重复。
> 全局安全红线和用户当前明确指令优先级高于本文件。
...
## Security And Secret Rules（项目特例，补充全局安全红线）
...
- 复杂任务拆成 3 到 7 个具体步骤，每个步骤必须可执行、可验证。
- 大范围修改（多文件重构、接口变更、数据模型调整）修改前后都应跑全量测试；无法执行必须说明原因。
- 纯文档修改可不跑测试，但必须说明这是文档改动且未验证运行时行为。
