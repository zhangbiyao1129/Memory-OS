...
// LLMMaintenanceCleaner 基于 LLM 的旧版候选整理器。
...
func (c LLMMaintenanceCleaner) Clean(ctx context.Context, candidates []Candidate) (CleanResult, error) {
	...
	if len(candidates) == 0 {
		return CleanResult{}, errors.New("no candidates to organize")
	}
...
func maintenanceCleanerSystemPrompt() string {
	return strings.TrimSpace(`你是 Memory OS 的候选记忆整理器。分析候选记忆列表,决定丢弃噪声、保留高价值候选。
