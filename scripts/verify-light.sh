...
QDRANT_BASE_URL="${QDRANT_BASE_URL:-http://127.0.0.1:18083}"
...
docker-compose -f deploy/docker-compose.yml -f deploy/docker-compose.t480.yml ps memory-api memory-worker memory-web memory-mcp qdrant >/dev/null
...
curl_json qdrant "$QDRANT_BASE_URL/collections"
